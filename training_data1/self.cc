...
continue;
// matched string, but not at a word boundary
}
}
}
...
{
return std::nullopt;
// line contains unknown token ID, cannot convert to string
}
...
if (line.empty())
continue;
// skip empty lines that can't be used for training
...
{
continue;
// skip too-short lines that can't be used for training
}
...
InputLine window;
int current_try_len = 2 + random_int(0, window_size - 2);
// random length between 2 and window_size
...
const size_t params_embed    = vocab * d_model;
// token embeddings
const size_t params_lm_head  = vocab * d_model;
// LM head
const size_t params_attn     = 4 * d_model * d_model;
// W_q, W_k, W_v, W_o per block
const size_t params_ffn      = 3 * d_model * d_ff;
// W_gate, W_up, W_down per block (2*d_ff*d_model + d_model*d_ff)
...
#else
// no OpenMP/FastFork: sequential execution
namespace parallel {
void init_parallel() {
...
size_t _num_layers = 2;
// overriden when loaded from file
Statistics stats;
...
// backwards compat: skip if absent
...
ws->d_h_mid = dout;
// copy first to avoid overwriting dout before it's used in dW_down
...
{
ws->d_raw.fill(RLMM_ZERO);
// d_raw accumulates per head; reset each iteration
...
if (t == b)
// last element  may race with steal_top
{
m_bottom.store(b + 1, std::memory_order_relaxed);
...
Context() = default;
~Context();
// defined in fastfork.cc; calls wait_local
...
// tasks run from thread's own queue
uint64_t tasks_stolen{0};
// tasks run stolen from another thread
uint64_t idle_polls{0};
// full steal_order scans that found nothing
uint64_t tasks_enqueued{0};
// tasks deposited into this thread's queue
};
...
// positions of the token in the corpus
std::vector<TokenID> m_tokens_in_file;
// the actual token IDs in the file, in order
};
...
size_t m_oi, m_ii;
// current outer/inner indices (relative to Enum::START)
...
{}
size_t m_pos;
// flat index in [0, m_end)
size_t m_end;
// total iterations = size1 * size2
size_t m_size2;
// stride of the inner dimension
};
}
// namespace rllm
#pragma once
...
WINDOW,
// sliding window of N tokens over flat corpus ((N-1) inputs  predict next)
};
const char* training_method_to_string(TrainingMethod method);
class NeuralNetwork
{
...
InputLine   m_last_input;
// saved in propagate_forward for use in propagate_backward
...
fixed_size_matrix<rlmm_float, TokenID, EmbeddingDimension> V_lm_head;
...
assert(RAND_MAX >= (2 * range + 1));
// sanity check to ensure RAND_MAX is sufficient
int offset = (rand() % (2 * range + 1)) - range;
// random value in [-range, range]
int raw_value = static_cast<int>(center) + offset;
int wrapped_value = (raw_value + static_cast<int>(T::MAX)) % static_cast<int>(T::MAX);
...
public:
TransformerBlock();
// defined in .cc after ForwardWorkspace is complete
~TransformerBlock();
// defined in .cc after ForwardWorkspace is complete
