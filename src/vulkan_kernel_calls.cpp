...
namespace rllm::vulkan
{
    ...
    ComputeKernelRuntime::~ComputeKernelRuntime()
    {
        ...
        if (m_timestamp_query_pool != VK_NULL_HANDLE && m_timestamp_query_pool_device != VK_NULL_HANDLE)
        {
            vkDestroyQueryPool(m_timestamp_query_pool_device, m_timestamp_query_pool, nullptr);
            m_timestamp_query_pool = VK_NULL_HANDLE;
        }
        if (m_command_buffer != VK_NULL_HANDLE && m_command_buffer_device != VK_NULL_HANDLE && m_command_pool != VK_NULL_HANDLE)
        {
    ...
    void ComputeKernelRuntime::wait_for_in_flight_submit()
    {
        ...
        {
            LOG_ERROR("vkWaitForFences failed for kernel '{}' ({}): {}", name(), spirv_path().string(), static_cast<int>(wait_result));
            std::abort();
        }
        m_submit_in_flight = false;
#if defined(RLLM_ENABLE_STATISTICS)
        if (m_pending_submit_has_timestamps && m_timestamp_query_pool != VK_NULL_HANDLE && m_timestamp_period_ns > 0.0f)
        {
            std::array<uint64_t, 2> timestamps{};
            const VkResult query_result = vkGetQueryPoolResults(
                m_timestamp_query_pool_device,
            ...
            {
                LOG_ERROR("vkGetQueryPoolResults failed for kernel '{}' ({}): {}", name(), spirv_path().string(), static_cast<int>(query_result));
                std::abort();
            }

            uint64_t elapsed_ticks = 0;
            bool has_elapsed_ticks = false;
            if (m_timestamp_valid_bits >= 64)
            {
                if (timestamps[1] >= timestamps[0])
                {
                    elapsed_ticks = timestamps[1] - timestamps[0];
                    has_elapsed_ticks = true;
                }
            }
            else if (m_timestamp_valid_bits > 0)
            {
                const uint64_t timestamp_mask = (uint64_t{1} << m_timestamp_valid_bits) - uint64_t{1};
                elapsed_ticks = (timestamps[1] - timestamps[0]) & timestamp_mask;
                has_elapsed_ticks = true;
            }

            if (has_elapsed_ticks)
            {
                const double elapsed_ns = static_cast<double>(elapsed_ticks) * static_cast<double>(m_timestamp_period_ns);
                const auto rounded_ns = static_cast<uint64_t>(elapsed_ns + 0.5);
                const std::string& timing_kernel_name = m_pending_submit_kernel_name.empty() ? m_name : m_pending_submit_kernel_name;
                parallel::statistics.record_kernel_timing(timing_kernel_name, std::chrono::nanoseconds(rounded_ns));
            }
        }
        m_pending_submit_has_timestamps = false;
        m_pending_submit_kernel_name.clear();
#endif
    }
    ...
    void ComputeKernelRuntime::ensure_pipeline(VkDevice device, uint32_t ssbo_binding_count)
    {
        ...
        if (m_cached_device != VK_NULL_HANDLE && (m_cached_device != device || m_ssbo_binding_count != ssbo_binding_count))
        {
            ...
            if (m_timestamp_query_pool != VK_NULL_HANDLE && m_timestamp_query_pool_device != VK_NULL_HANDLE)
            {
                vkDestroyQueryPool(m_timestamp_query_pool_device, m_timestamp_query_pool, nullptr);
                m_timestamp_query_pool = VK_NULL_HANDLE;
                m_timestamp_query_pool_device = VK_NULL_HANDLE;
        ...
        if (m_shader_module == VK_NULL_HANDLE)
        {
            ...
            {
                LOG_ERROR("vkCreateShaderModule failed for kernel '{}' ({}): {}", m_name, m_spirv_path.string(), static_cast<int>(shader_result));
                std::abort();
            }
        }
        ...
        if (m_descriptor_set_layout == VK_NULL_HANDLE && ssbo_binding_count > 0)
        {
            ...
            const VkResult set_layout_result = vkCreateDescriptorSetLayout(device, &set_layout_info, nullptr, &m_descriptor_set_layout);
            if (set_layout_result != VK_SUCCESS)
            {
                LOG_ERROR("vkCreateDescriptorSetLayout failed for kernel '{}' ({}): {}", m_name, m_spirv_path.string(), static_cast<int>(set_layout_result));
                std::abort();
            }
            ...
            {
                LOG_ERROR("vkCreateDescriptorPool failed for kernel '{}' ({}): {}", m_name, m_spirv_path.string(), static_cast<int>(pool_result));
                std::abort();
            }
            ...
            {
                LOG_ERROR("vkAllocateDescriptorSets failed for kernel '{}' ({}): {}", m_name, m_spirv_path.string(), static_cast<int>(alloc_set_result));
                std::abort();
            }
        }
        ...
        if (m_pipeline_layout == VK_NULL_HANDLE)
        {
            ...
            if (layout_result != VK_SUCCESS)
            {
                ...
                m_shader_module = VK_NULL_HANDLE;
                LOG_ERROR("vkCreatePipelineLayout failed for kernel '{}' ({}): {}", m_name, m_spirv_path.string(), static_cast<int>(layout_result));
                std::abort();
            }
        }
        ...
        if (m_pipeline == VK_NULL_HANDLE)
        {
            ...
            const VkResult pipeline_result = vkCreateComputePipelines(device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &m_pipeline);
            if (pipeline_result != VK_SUCCESS)
            {
                ...
                m_pipeline_layout = VK_NULL_HANDLE;
                m_shader_module = VK_NULL_HANDLE;
                LOG_ERROR("vkCreateComputePipelines failed for kernel '{}' ({}): {}", m_name, m_spirv_path.string(), static_cast<int>(pipeline_result));
                std::abort();
            }
        }
    ...
    void ComputeKernelRuntime::dispatch_kernel(uint32_t groups_x, uint32_t groups_y, uint32_t groups_z, std::span<const detail::HostBufferView> buffers, std::span<const std::byte> push_constants, std::span<ComputeKernelRuntime::RuntimeBuffer> runtime_buffers, size_t& runtime_buffer_count)
    {
        ...
        wait_for_in_flight_submit();
        const uint32_t ssbo_binding_count = std::max<uint32_t>(m_parsed_ssbo_binding_count, static_cast<uint32_t>(buffers.size()));
        if (buffers.size() != ssbo_binding_count)
        {
            LOG_ERROR("Kernel '{}' expected {} SSBO buffers, but launch provided {}.", name(), ssbo_binding_count, buffers.size());
            std::abort();
        }
        ...
        {
            LOG_ERROR("Kernel '{}' needs {} runtime buffers, but storage only has {}.", name(), buffers.size(), runtime_buffers.size());
            std::abort();
        }
        ...
        for (size_t buf_idx = 0; buf_idx < buffers.size(); ++buf_idx)
        {
            ...
            auto seed_runtime_buffer = [&](RuntimeBuffer& runtime_buffer) {
                ...
                if (view.offload_ptr != nullptr && (owner == DeviceMemoryOwner::ON_DEVICE || owner == DeviceMemoryOwner::REPLICATED))
                {
                    if (!detail::resolve_offset(ctx.offload_base, ctx.offload_size, view.offload_ptr, view.size_bytes, runtime_buffer.offload_source_offset))
                    {
                        LOG_ERROR("Unable to resolve persistent offload offset for kernel '{}' parameter '{}'.", name(), view.parameter_name.empty() ? std::string_view{"<unnamed>"} : view.parameter_name);
                        cleanup_runtime_buffers();
                        std::abort();
                    }
                    runtime_buffer.use_offload_source = true;
                    const VkDeviceSize offset_alignment = ctx.storage_buffer_offset_alignment == 0 ? 1 : ctx.storage_buffer_offset_alignment;
                    runtime_buffer.bind_offload_direct = runtime_buffer.offload_source_offset % offset_alignment == 0;
                    return;
                }
                ...
                {
                    IMemorySpace::get_instance()->copy_staging_to_offload(const_cast<void*>(view.offload_ptr), runtime_buffer.view.host_ptr, runtime_buffer.view.size_bytes);
            ...
            {
                LOG_ERROR("Refusing to allocate Vulkan buffer for kernel '{}' parameter '{}' with invalid size {}.", name(), view.parameter_name.empty() ? std::string_view{"<unnamed>"} : view.parameter_name, view.size_bytes);
                cleanup_runtime_buffers();
                std::abort();
            }
            ...
            if (keep_runtime_buffer)
            {
                ...
                if (stored.device == ctx.device && stored.buffer != VK_NULL_HANDLE && stored.memory != VK_NULL_HANDLE && stored.staging_buffer != VK_NULL_HANDLE && stored.staging_memory != VK_NULL_HANDLE && stored.mapped != nullptr && stored.size_bytes >= view.size_bytes)
                {
            ...
            create_runtime_buffer_resource(static_cast<VkDeviceSize>(view.size_bytes), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, rb.buffer, rb.memory);
            create_runtime_buffer_resource(static_cast<VkDeviceSize>(view.size_bytes), VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, rb.staging_buffer, rb.staging_memory);
        ...
        if (m_submit_fence == VK_NULL_HANDLE || m_submit_fence_device != ctx.device)
        {
            ...
            {
                LOG_ERROR("vkCreateFence failed for kernel '{}' ({}): {}", name(), spirv_path().string(), static_cast<int>(fence_result));
                std::abort();
            }
        ...
        if (m_command_buffer == VK_NULL_HANDLE)
        {
            ...
            if (alloc_result != VK_SUCCESS)
            {
                LOG_ERROR("vkAllocateCommandBuffers failed for kernel '{}' ({}): {}", name(), spirv_path().string(), static_cast<int>(alloc_result));
                std::abort();
            }
        ...
        else
        {
            ...
            {
                LOG_ERROR("vkResetCommandBuffer failed for kernel '{}' ({}): {}", name(), spirv_path().string(), static_cast<int>(reset_result));
                std::abort();
            }
        }

#if defined(RLLM_ENABLE_STATISTICS)
        if ((m_timestamp_query_pool == VK_NULL_HANDLE || m_timestamp_query_pool_device != ctx.device) && m_timestamp_period_ns >= 0.0f)
        {
            if (m_timestamp_query_pool != VK_NULL_HANDLE && m_timestamp_query_pool_device != VK_NULL_HANDLE)
            {
                vkDestroyQueryPool(m_timestamp_query_pool_device, m_timestamp_query_pool, nullptr);
            ...
            vkGetPhysicalDeviceQueueFamilyProperties(ctx.physical_device, &queue_family_count, queue_family_props.data());
            m_timestamp_valid_bits = ctx.queue_family_index < queue_family_props.size()
                ? queue_family_props[ctx.queue_family_index].timestampValidBits
                : 0;

            if (m_timestamp_period_ns > 0.0f && m_timestamp_valid_bits > 0)
            {
                ...
                const VkResult query_pool_result = vkCreateQueryPool(ctx.device, &query_pool_info, nullptr, &m_timestamp_query_pool);
                if (query_pool_result != VK_SUCCESS)
                {
                    LOG_ERROR("vkCreateQueryPool failed for kernel '{}' ({}): {}", name(), spirv_path().string(), static_cast<int>(query_pool_result));
                    std::abort();
                }
                m_timestamp_query_pool_device = ctx.device;
            }
            else
            {
        ...
        {
            LOG_ERROR("vkBeginCommandBuffer failed for kernel '{}' ({}): {}", name(), spirv_path().string(), static_cast<int>(begin_result));
            std::abort();
        }
        ...
        {
            vkCmdPushConstants(m_command_buffer, m_pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, static_cast<uint32_t>(push_constants.size()), push_constants.data());
        }
        ...
        if (runtime_buffer_count > 0)
        {
            if (ctx.offload_buffer != VK_NULL_HANDLE && ctx.offload_size != 0)
            {
                ...
                offload_visibility_barrier.dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT | VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT;
                ...
                vkCmdPipelineBarrier(m_command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT | VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT | VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0, nullptr, 1, &offload_visibility_barrier, 0, nullptr);
            }
            ...
            for (size_t i = 0; i < runtime_buffer_count; ++i)
            {
                ...
                const VkBuffer upload_src_buffer = runtime_buffers[i].use_offload_source ? ctx.offload_buffer : runtime_buffers[i].staging_buffer;
            ...
            {
                vkCmdPipelineBarrier(m_command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0, nullptr, static_cast<uint32_t>(upload_barriers.size()), upload_barriers.data(), 0, nullptr);
            }
        ...
        if (runtime_buffer_count > 0)
        {
            ...
            for (size_t i = 0; i < runtime_buffer_count; ++i)
            {
                ...
                if (runtime_buffers[i].view.offload_ptr != nullptr && !runtime_buffers[i].bind_offload_direct)
                {
                    ...
                    if (!detail::resolve_offset(ctx.offload_base, ctx.offload_size, runtime_buffers[i].view.offload_ptr, runtime_buffers[i].view.size_bytes, offload_dst_offset))
                    {
                        LOG_ERROR("Unable to resolve persistent offload destination offset for kernel '{}' parameter '{}'.", name(), runtime_buffers[i].view.parameter_name.empty() ? std::string_view{"<unnamed>"} : runtime_buffers[i].view.parameter_name);
                        cleanup_runtime_buffers();
                        std::abort();
                    }
            ...
            {
                vkCmdPipelineBarrier(m_command_buffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, static_cast<uint32_t>(download_source_barriers.size()), download_source_barriers.data(), 0, nullptr);
        ...
        {
            LOG_ERROR("vkEndCommandBuffer failed for kernel '{}' ({}): {}", name(), spirv_path().string(), static_cast<int>(end_result));
            std::abort();
        }
        ...
        {
            LOG_ERROR("vkResetFences failed for kernel '{}' ({}): {}", name(), spirv_path().string(), static_cast<int>(reset_fence_result));
            std::abort();
        }
...
#if defined(RLLM_ENABLE_STATISTICS)
        m_pending_submit_kernel_name = name();
        m_pending_submit_has_timestamps = m_timestamp_query_pool != VK_NULL_HANDLE;
#endif

        const VkResult submit_result = vkQueueSubmit(ctx.queue, 1, &submit_info, m_submit_fence);
        if (submit_result != VK_SUCCESS)
        {
            LOG_ERROR("vkQueueSubmit failed for kernel '{}' ({}): {}", name(), spirv_path().string(), static_cast<int>(submit_result));
            std::abort();
        }
