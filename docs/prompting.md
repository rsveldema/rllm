...
quoted literals become `<STRING>`, and calls, imports, includes, or qualified
accesses are marked with `<MCP>` and `</MCP>`. Source text does not need to
contain these tokens explicitly; already-normalized markers are still accepted.
