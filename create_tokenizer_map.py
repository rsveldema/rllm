...
def create_tokenizer_map(text, support_extra_latin_characters: bool = False) -> dict[str, int]:
    ...
    tokens = [
        ...
        "<MCP>",
        "</MCP>",
        *(f"<LOOP_{index}>" for index in range(8)),
        "<LOOP_OVERFLOW>",
        *(f"<LOCAL_{index}>" for index in range(16)),
        "<LOCAL_OVERFLOW>",
        *(f"<PARAM_{index}>" for index in range(16)),
        "<PARAM_OVERFLOW>",
        *(f"<GLOBAL_{index}>" for index in range(16)),
        "<GLOBAL_OVERFLOW>",
        "<FIELD_ACCESS_IDENT>",
        "<STRING>",
    ]
    ...
    # Corpus-derived BPE pieces would encode concrete names and literal
    # vocabulary. Keywords, their prefixes, and characters provide fallback
    # coverage without teaching the model project or library identifiers.
...
def generate_cpp_table_impl(tokenizer_map) -> str:
    ...
    cpp_table += "};\n"
    keyword_sets = {
        "cpp": CPP_KEYWORDS + PREPROCESSOR_KEYWORDS,
        "c": CPP_KEYWORDS + PREPROCESSOR_KEYWORDS,
        "python": PYTHON_KEYWORDS,
        "rust": RUST_KEYWORDS,
        "java": JAVA_KEYWORDS,
        "shell": SHELL_KEYWORDS,
    }
    cpp_table += "\nbool is_language_keyword(std::string_view word, std::string_view language)\n{\n"
    for language, keywords in keyword_sets.items():
        conditions = " || ".join(f'word == "{keyword}"' for keyword in dict.fromkeys(keywords))
        cpp_table += f'    if (language == "{language}") return {conditions};\n'
    cpp_table += "    return false;\n}\n"
    cpp_table += "\n} // namespace rllm\n"
    return cpp_table
...
def generate_cpp_table_header(tokenizer_map) -> str:
    ...
    for _token, idx in tokenizer_map.items():
        ...
        language_aliases = {
            ...
            "<MCP>": "MCP_START",
            "</MCP>": "MCP_END",
            **{f"<LOOP_{index}>": f"LOOP_{index}" for index in range(8)},
            "<LOOP_OVERFLOW>": "LOOP_OVERFLOW",
            **{f"<LOCAL_{index}>": f"LOCAL_{index}" for index in range(16)},
            "<LOCAL_OVERFLOW>": "LOCAL_OVERFLOW",
            **{f"<PARAM_{index}>": f"PARAM_{index}" for index in range(16)},
            "<PARAM_OVERFLOW>": "PARAM_OVERFLOW",
            **{f"<GLOBAL_{index}>": f"GLOBAL_{index}" for index in range(16)},
            "<GLOBAL_OVERFLOW>": "GLOBAL_OVERFLOW",
            "<FIELD_ACCESS_IDENT>": "FIELD_ACCESS_IDENT",
            "<STRING>": "STRING",
        }
    ...
    cpp_table += "bool is_language_keyword(std::string_view word, std::string_view language);\n"
    cpp_table += "\n} // namespace rllm\n"
    return cpp_table
