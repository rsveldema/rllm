...
namespace rllm
{
    ...
    void accumulate_optimizer_gradient_norm(const fixed_size_matrix<float, IdentifierHashBucket, EmbeddingDimension>& gradient, fixed_size_vector<float, TempStorage>& values);
    ...
    void accumulate_optimizer_gradient_norm(const fixed_size_matrix<float, PositionIndex, EmbeddingDimension>& gradient, PositionIndex rows, fixed_size_vector<float, TempStorage>& values);
