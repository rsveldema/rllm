...
namespace rllm
{
    /** Fixed-capacity vector of heap-allocated objects. CPU-only (no GPU offload). */
    template <typename T, typename LengthType>
    class cpu_fixed_obj_vector
    {
      public:
        static constexpr size_t CAPACITY = static_cast<size_t>(LengthType::MAX);

        cpu_fixed_obj_vector()
        {
            allocate_slots();
        }

        cpu_fixed_obj_vector(const cpu_fixed_obj_vector& other)
            : len(other.len)
        {
            allocate_slots();
        ...
        cpu_fixed_obj_vector& operator=(const cpu_fixed_obj_vector& other)
        {
            if (this != &other)
            {
                ensure_slots();
        ...
        cpu_fixed_obj_vector(cpu_fixed_obj_vector&&) noexcept = default;
        cpu_fixed_obj_vector& operator=(cpu_fixed_obj_vector&&) noexcept = default;
        ~cpu_fixed_obj_vector() = default;
      ...
      private:
        ...
        void copy_slots_from(const cpu_fixed_obj_vector& other)
        {
